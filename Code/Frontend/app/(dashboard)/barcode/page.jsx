    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {soundEnabled ? (
              <Volume2 className="mr-1.5 h-4 w-4 text-indigo-400" />
            ) : (
      ...
      <Card className="relative overflow-hidden bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-indigo-950/40 border border-indigo-500/20 shadow-2xl shadow-indigo-500/10 rounded-3xl">
          ...
          <div className="flex flex-col items-center gap-6">
            <div className="flex flex-wrap items-center justify-center gap-4">
              <button
                ...
                className="flex flex-col items-center justify-center rounded-2xl bg-purple-500/20 p-5 text-purple-300 hover:bg-purple-500/30 hover:scale-105 transition-all duration-200 focus:outline-none cursor-pointer border border-purple-500/30 min-w-[130px] shadow-lg shadow-purple-500/10"
                title="Use Live Camera Scanner"
              >
                <Camera className="h-8 w-8 mb-2 text-purple-400" />
              ...
              <button
                ...
                className="flex flex-col items-center justify-center rounded-2xl bg-indigo-500/20 p-5 text-indigo-300 hover:bg-indigo-500/30 hover:scale-105 transition-all duration-200 focus:outline-none cursor-pointer border border-indigo-500/30 min-w-[130px] shadow-lg shadow-indigo-500/10"
            ...
            <div className="flex w-full max-w-xl gap-3">
              <div className="relative flex-1 group">
                <ScanBarcode className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-indigo-400 transition-colors" />
                <Input
                  placeholder="Scan or type barcode / SKU..."
                  value={barcodeInput}
                  onChange={(e) => setBarcodeInput(e.target.value)}
                  onKeyDown={handleKeyPress}
                  className="pl-11 h-12 bg-slate-950/70 border-white/10 text-slate-100 placeholder:text-slate-500 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 rounded-xl text-base font-mono"
                  autoFocus
                />
              </div>
              <Button
                onClick={handleScan}
                disabled={isScanning}
                className="h-12 px-6 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold rounded-xl shadow-lg shadow-indigo-500/20"
              >
                {isScanning ? (
                  "Looking up..."
                ) : (
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-1 text-xs">
              ...
              {["SKU-001", "SKU-002", "12345678", "PROD-100"].map((sampleSku) => (
                <button
                  ...
                  className="rounded-full border border-indigo-500/30 bg-indigo-500/10 px-3 py-1 text-indigo-300 hover:bg-indigo-500/20 hover:text-white hover:scale-105 transition-all font-mono font-medium"
                >
                  {sampleSku}
                </button>
              ))}
      ...
      {showCamera && (
        <Card className="border-indigo-500/30 bg-slate-900/90 p-6 shadow-2xl rounded-2xl">
          <CardContent className="flex flex-col items-center justify-center p-0 relative">
              ...
              <div className="flex items-center gap-2">
                ...
                {cameras.length > 1 && (
                  <select
                    ...
                    className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-1 text-xs text-slate-200 focus:outline-none"
                  >
                    {cameras.map((cam) => (
            ...
            {cameraError ? (
              <div className="flex flex-col items-center justify-center p-8 text-center max-w-md bg-slate-950/60 rounded-xl border border-rose-500/20">
              ...
              <>
                <div
                  id="camera-reader"
                  className="w-full max-w-sm rounded-xl overflow-hidden border border-slate-800 bg-black min-h-[260px] shadow-inner"
                ></div>
                {isCameraLoading && (
      ...
      {batchMode && (
        <Card className="border-emerald-500/30 bg-emerald-500/5 backdrop-blur-xl rounded-2xl">
            ...
            {Object.keys(batchItems).length === 0 ? (
              ...
              <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                {Object.values(batchItems).map((item) => (
                  <div
                    key={item.sku}
                    className="flex items-center justify-between p-3 rounded-xl bg-slate-900/80 border border-emerald-500/20"
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-base font-bold px-3 py-1">
                        ×{item.count}
                      </Badge>
                    </div>
                  </div>
          ...
          <CardContent className="space-y-2">
            ...
            {history.map((h, idx) => (
              <div
                ...
                className="flex items-center justify-between rounded-xl border border-white/5 bg-slate-900/50 px-3.5 py-2.5 text-sm hover:border-white/10 transition-colors"
              >
                <div>
                ...
                {h.result === "found" ? (
                  <Badge className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                    <CheckCircle className="mr-1 h-3 w-3 text-emerald-400" />{" "}
                    {h.name || "Found"}
                  </Badge>
                ) : (
                  <Badge className="bg-rose-500/20 text-rose-300 border border-rose-500/30">
                    <AlertTriangle className="mr-1 h-3 w-3 text-rose-400" /> Not
                    found
                  </Badge>
                )}
              </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Printer className="h-5 w-5 text-indigo-400" /> Barcode Bin Label Preview
            </DialogTitle>
            <DialogDescription>
