...
export function AbcStochasticPanel({ data = [], className = "" }) {
  ...
  return (
    <Card className={`border border-white/5 bg-slate-900/40 backdrop-blur-2xl shadow-xl rounded-2xl relative w-full max-w-full overflow-hidden ${className}`}>
      ...
      <CardHeader className="p-6 border-b border-white/5 pb-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <Select value={abcFilter} onValueChange={setAbcFilter}>
              <SelectTrigger className="w-[160px] bg-slate-950/50 border-white/10 text-slate-200 rounded-xl h-10">
          ...
          <Table className="w-full min-w-[850px]">
            <TableHeader className="bg-slate-950/60 border-b border-slate-800">
              <TableRow className="hover:bg-transparent border-slate-800">
                <TableHead className="py-3 px-4 font-semibold text-slate-300 text-xs uppercase tracking-wider">Product Info</TableHead>
                <TableHead className="py-3 px-4 font-semibold text-slate-300 text-xs uppercase tracking-wider">Category</TableHead>
                <TableHead className="py-3 px-4 font-semibold text-slate-300 text-xs uppercase tracking-wider text-center">ABC/XYZ Matrix</TableHead>
                <TableHead className="py-3 px-4 font-semibold text-slate-300 text-xs uppercase tracking-wider text-center">Stochastic SS / ROP</TableHead>
                <TableHead className="py-3 px-4 font-semibold text-slate-300 text-xs uppercase tracking-wider text-center">Service Level</TableHead>
                <TableHead className="py-3 px-4 font-semibold text-slate-300 text-xs uppercase tracking-wider text-right">Buffer Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredData.length > 0 ? (
                filteredData.map((p) => {
                  ...
                  if (stock <= 0) {
                    bufferBadge = (
                      <Badge variant="outline" className="bg-rose-500/10 text-rose-400 border-rose-500/30 text-[11px] gap-1">
                  ...
                  return (
                    <TableRow key={p.id} className="hover:bg-slate-800/40 transition-colors border-b border-slate-800/60">
                      ...
                      <TableCell className="py-3.5 px-4 text-xs text-slate-400">
                      ...
                      <TableCell className="py-3.5 px-4 text-center">
                        <div className="inline-flex flex-col items-center gap-0.5">
                          ...
                          <span className="text-[10px] text-slate-400 capitalize">{policy}</span>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="py-3.5 px-4 text-center">
                        <Badge variant="outline" className="bg-cyan-500/10 text-cyan-300 border-cyan-500/30 font-mono text-xs">
                          <ShieldCheck className="h-3 w-3 mr-1 text-cyan-400" /> {sl}% Target SL
                        </Badge>
                      </TableCell>
